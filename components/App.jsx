...
function BracketDisplay({bracket,currentPlayer}){
  ...
  return(<div>
    ...
    <div style={{overflowX:"auto",paddingBottom:16}}>
      {showF4?(<div style={{padding:"8px 0"}}><Lbl>First Four — 1pt each</Lbl><div style={{display:"grid",gridTemplateColumns:"1fr",gap:10,maxWidth:260}}>{(()=>{const ffGames=bracket.play_in||[];const ffLocked=ffGames.some(g=>g.status==="live"||g.status==="final"||(g.tipoff_time&&new Date(g.tipoff_time)<=new Date()));return ffGames.map((g,i)=><GameCell key={i} game={g} roundIdx={0} currentPlayer={currentPlayer} allPlayers={allPlayers} roundLocked={ffLocked}/>);})()}</div></div>)
