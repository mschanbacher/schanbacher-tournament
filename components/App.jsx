...
function RegionBracket({games,currentPlayer,allPlayers}){
  ...
  return(<div style={{position:"relative",display:"flex",minHeight:totalH}}>
    {games.map((round,ri)=>(<div key={ri} style={{position:"relative",width:COL,flexShrink:0}}><div style={{fontSize:9,letterSpacing:2,color:C.textLight,textTransform:"uppercase",fontWeight:600,marginBottom:8}}>{labels[ri]}</div><div style={{position:"relative"}}>{round.map((g,gi)=>(<div key={gi} style={{position:"absolute",top:pos[ri][gi],left:0}}><GameCell game={g} roundIdx={ri+1} currentPlayer={currentPlayer} allPlayers={allPlayers} roundLocked={games[ri].some(gg=>gg&&(gg.status==="live"||gg.status==="final"||(gg.tipoff_time&&new Date(gg.tipoff_time)<=new Date())))}/></div>))}</div></div>))}
...
function UpsetsTab({tournaments,mob}){
  ...
  useEffect(()=>{
    (async()=>{
      ...
      // Best single-year upset callers
      const bestYears=[];
      for(const ys of yearStats){
        for(const player of PLAYERS_ALL){
...
function StreaksTab({completedResults,tournaments,mob}){
  ...
  useEffect(()=>{
    (async()=>{
      ...
      for(const year of allYears){
        // Get games ordered by tipoff time (chronological), fallback to round+game_order
        ...
        // For each player, build the pick sequence in chronological order
        ...
        for(const player of PLAYERS_ALL){
          ...
          for(let i=0;i<sequence.length;i++){
            if(sequence[i].correct){
              ...
              if(i===startFromGame1||i===startFromGame1+curCorrect-1){}// part of opening streak
          ...
          // Best start: consecutive correct from game 0
          let bestStart=0;
          ...
          // Pending games for visualization
  ...
  // All-time records: aggregate across years, find top streaks
  // For cross-year records, we need per-year max streaks
  ...
  // Perfect rounds: find any player-year where round score equals max
...
function RecordsView({seasonResults,tournaments,mob}){
  ...
  return(<div style={{padding:mob?"20px 16px":"32px 40px",maxWidth:900,margin:"0 auto"}}><div style={{marginBottom:24}}><Lbl>Since 2008</Lbl><h2 style={{fontSize:28,color:C.text,margin:"4px 0",fontWeight:700,lineHeight:1}}>Records</h2></div>
    ...
    {tab==="upsets"?(<UpsetsTab tournaments={tournaments} mob={mob}/>):tab==="streaks"?(<StreaksTab completedResults={completedResults} tournaments={tournaments} mob={mob}/>):tab==="rounds"?(<div>{roundRecords.map((rr,ri)=>{const hdr={textAlign:"right",padding:"6px 8px",fontSize:10,color:C.textLight,letterSpacing:1,fontWeight:600};const cv={textAlign:"right",padding:"8px 8px",fontSize:13,fontVariantNumeric:"tabular-nums",fontWeight:600};const nv={textAlign:"right",padding:"8px 4px",fontSize:11,fontVariantNumeric:"tabular-nums",color:C.textLight,fontWeight:400,width:24};return(<div key={ri} style={{marginBottom:32}}>
