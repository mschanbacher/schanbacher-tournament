...
function RegionBracket({games,currentPlayer,allPlayers,roundLocks}){
...
function BracketView({currentPlayer,activeYear,mob}){
  ...
  const isRoundTipped=(gms)=>gms.some(g=>g&&(g.status==="live"||g.status==="final"||(g.tipoff&&new Date(g.tipoff)<=new Date())));
  const regionRoundLocks={1:isRoundTipped(Object.values(bracket.regions).flatMap(r=>r.r1)),2:isRoundTipped(Object.values(bracket.regions).flatMap(r=>r.r2)),3:isRoundTipped(Object.values(bracket.regions).flatMap(r=>r.s16)),4:isRoundTipped(Object.values(bracket.regions).flatMap(r=>r.e8))};
...
function GamesTab({tournaments,mob}){
