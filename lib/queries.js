...
export async function fetchBracketForYear(year) {
  ...
  // If no picks exist yet, players will be empty — caller should use fetched players list
  for (const r of (regions || [])) bracket.regions[r.name] = { r1: [], r2: [], s16: [], e8: [] }
  const roundKeyMap = { 1: 'r1', 2: 'r2', 3: 's16', 4: 'e8' }
  for (const g of (games || [])) {
    const game = { id: g.id, s1: g.seed1, t1: g.team1, s2: g.seed2, t2: g.team2, sc1: g.score1, sc2: g.score2, w: g.winner, picks: picksMap[g.id] || {}, status: g.status, tipoff: g.tipoff_time, espnId: g.espn_id, statusDetail: g.status_detail }
